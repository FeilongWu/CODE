...
class Diffusion(nn.Module):
    def __init__(self, n_layers, diffusion_time_embedding_dim,
                 latent_dim,diff_embed_dim,vital_sign_dim,
                 n_times=1000, beta_minmax=[1e-4, 2e-2],
                 device='cuda',cls_emb_dim=1024,
                 ts_cond=True,highest_percent=0.3,
                 lambda1=1.0, 
                 lambda0=0.5,
                 tp_embed_dim=128,
                 tp_layer=2):
        ...
        self.model = Denoiser(self.cls_emb_dim, latent_dim,diff_embed_dim, n_layers,
    ...
    def loss(self, x_cls,x_cls_inc, mask_txt,
             x0,tp_densities,cond=None,pretrain=False):
        # x_cls, report cls embeddings with padded data, shape=[bs,seq_len, 1024]
        # x_cls_inc, incomplete report cls embeddings with padded data, shape=[bs, seq_len, 1024]
        # t_txt: timestamps, shape=[bs, seq_len]
        # mask_txt: 1 non-padded, 0 --- padded report, shape=[bs, seq_len]
    ...
    def rank_mask(self, x, mask, percent):
        # x: [bs, 1]
        # mask: 1 = non-pad, 0 = pad
        mask1 = mask.view(-1, 1)
        n = int(torch.sum(mask1).item())
        x1 = x * mask1
        sort_x = torch.sort(x.flatten())[0]
        idx = int(n * (1.0-percent)) + (len(x) - n)
        lowest = sort_x[idx]
        mask = x >= lowest
    ...
    def loss_complete(self, x_cls,x0, tp_densities_txt,tp_densities_cor,
        ...
        tp_0_mask = self.rank_mask(p_tp_0, mask_txt, self.highest_percent)
        loss1 = mask_txt * \
